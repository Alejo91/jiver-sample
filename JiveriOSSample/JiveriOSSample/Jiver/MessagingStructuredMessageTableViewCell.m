...
{
    self.message = model;
    JiverSender *sender = [self.message sender];
    long long ts = [self.message getMessageTimestamp] / 1000;
    [self.dateTimeLabel setText:[JiverUtils messageDateTime:ts]];
...
#warning THIS IS SAMPLE CODE. Do not use ImageCache in your product. Use your own image loader or 3rd party image loader.
    ...
    if (image) {
        ...
        [JiverUtils imageDownload:[NSURL URLWithString:[sender imageUrl]] endBlock:^(NSData *response, NSError *error) {
            UIImage *image = [[UIImage alloc] initWithData:response scale:1];
            UIImage *newImage = [JiverUtils imageWithImage:image scaledToSize:kFileLinkProfileWidth];
...
#warning THIS IS SAMPLE CODE. Do not use ImageCache in your product. Use your own image loader or 3rd party image loader.
    ...
    if (iconImage) {
        ...
        [JiverUtils imageDownload:[NSURL URLWithString:[self.message structuredMessageIconUrl]] endBlock:^(NSData *response, NSError *error) {
            UIImage *image = [[UIImage alloc] initWithData:response scale:1];
            //            UIImage *newImage = [JiverUtils imageWithImage:image scaledToSize:198];
...
#warning THIS IS SAMPLE CODE. Do not use ImageCache in your product. Use your own image loader or 3rd party image loader.
    ...
    if (image) {
        ...
        [JiverUtils imageDownload:[NSURL URLWithString:[self.message structuredMessageThumbUrl]] endBlock:^(NSData *response, NSError *error) {
            UIImage *image = [[UIImage alloc] initWithData:response scale:1];
//            UIImage *newImage = [JiverUtils imageWithImage:image scaledToSize:kFileLinkWidth];
            UIImage *newImage = image;

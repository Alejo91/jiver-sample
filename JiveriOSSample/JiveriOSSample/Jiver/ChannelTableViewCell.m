...
- (void) setModel:(JiverChannel *)model
{
    ...
    [self.channelUrlLabel setText:[NSString stringWithFormat:@"#%@",[JiverUtils getChannelNameFromUrl:[model url]]]];
    ...
    if ([[Jiver getCurrentChannel] channelId] == [model channelId]) {
...
#warning THIS IS SAMPLE CODE. Do not use ImageCache in your product. Use your own image loader or 3rd party image loader.
    ...
    if (image) {
        ...
        [JiverUtils imageDownload:[NSURL URLWithString:[model coverUrl]] endBlock:^(NSData *response, NSError *error) {
            UIImage *image = [[UIImage alloc] initWithData:response scale:1];
            UIImage *newImage = [JiverUtils imageWithImage:image scaledToSize:38];

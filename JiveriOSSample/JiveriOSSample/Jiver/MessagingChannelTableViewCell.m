...
- (void) setModel:(JiverMessagingChannel *)model withCheckMark:(BOOL)check
{
    ...
    if (check) {
        ...
        if ([model lastMessage] == nil) {
            ...
            [self.lastMessageDateLabel setText:[JiverUtils lastMessageDateTime:ts]];
        }
    ...
    [self.profileImageView setImageWithURL:[NSURL URLWithString:[JiverUtils getDisplayCoverImageUrl:[model members]]]];
#else
#warning THIS IS SAMPLE CODE. Do not use ImageCache in your product. Use your own image loader or 3rd party image loader.
    UIImage *image = [[ImageCache sharedInstance] getImage:[JiverUtils getDisplayCoverImageUrl:[model members]]];
    if (image) {
        @try {
        ...
        NSString *imageUrl = [JiverUtils getDisplayCoverImageUrl:[model members]];
